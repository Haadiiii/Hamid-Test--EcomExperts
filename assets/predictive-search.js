class PredictiveSearch extends SearchForm {
  ...
  updateSearchForTerm(previousTerm, newTerm) {
    ...
    if (currentButtonText) {
      // Check if the match exists before calling match
      const matchExists = currentButtonText.includes(previousTerm);

      if (
        matchExists &&
        currentButtonText.match(new RegExp(previousTerm, "g")).length > 1
      ) {
  ...
  renderSearchResults(resultsMarkup) {
    const modifiedResultsMarkup = this.excludeSoftWinterJacket(resultsMarkup);
    this.predictiveSearchResults.innerHTML = modifiedResultsMarkup;
  ...
  excludeSoftWinterJacket(resultsMarkup) {
    // Assuming the product name is included in the search results markup
    // Modify this to match the actual structure of your search results
    const searchTerm = "Soft Winter Jacket";
    if (resultsMarkup.includes(searchTerm)) {
      // show nothing if the search term is found
      return "";
    } else {
